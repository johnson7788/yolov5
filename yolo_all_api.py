...
class YOLOModel(object):
    def __init__(self, verbose=0):
        ...
        self.label_list = ['table','figure','equation','algorithm','title','paragraph']
        self.label_list_cn = ['表格','图像','公式','算法','标题','段落']
        #给每个类别的候选框设置一个颜色
        ...
        #模型的名称或路径
        self.weights = 'runs/train/paragraph_title/weights/best.pt'      # 'yolov5s.pt'
    ...
    def detect(self, data):
        ...
        for idx, (path, img, im0s, vid_cap) in enumerate(dataset):
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                #p原始图片的绝对路径
                p = Path(p)  # to Path
                save_path = os.path.join(self.predict_dir, 'pt_'+p.name)  #预测后的保存的图片的路径
